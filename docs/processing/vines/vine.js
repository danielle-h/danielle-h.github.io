/**
 * sketch for showing a growing vine.
 *
...
class Vine {
  ...
  spawn() {
    if (this.rad > 5) {
      //don't create if too small, looks weird.
      if (this.x > 0 && this.x < width && this.y > 0 && this.y < height) {
        //don't create if out of screen
        ...
        if (left) {
          return new Vine(
            this.x,
            this.y,
            random(this.angle - HALF_PI / 2, this.angle - HALF_PI / 4),
            min(this.rad, 5),
            this.g,
            this.b
          );
          ...
          return new Vine(
            this.x,
            this.y,
            random(this.angle + HALF_PI / 4, this.angle + HALF_PI / 2),
            min(this.rad, 5),
            this.g,
            this.b
          );
  ...
  draw() {
    ...
    //console.log(this.rad)
    if (this.currTime - this.previousTime > this.delta) {
      //draw if time passed
